...
# 2 表达式求值 {#2.expression-evaluation}
...
## 2.1 赋值 - 引用和值 {#2.1.reference-and-value}
...
## 2.2 赋值 - 装箱和拆箱 {#2.2.box-and-unbox}
...
## 2.3 赋值 - 多路赋值 {#2.3.multiple-assignments}
...
## 2.4 表达式里的顺序问题&数学的等值关系 {#2.4.ordering-within-expressions}
...
## 2.5 短路求值 {#2.5.short-circuit-evaluation}
...
# 3 结构化和非结构化的流程 {#3.structured-and-unstructured-flow}
...
## 3.1 goto的结构化替代品 {#3.1.structured-alternatives-to-goto}
...
## 3.2 继续(Continuations) {#3.2.continuations}
...
# 4 顺序复合(Sequencing) {#4.sequencing}
...
## 5.1 短路条件 {#5.1.short-circuited-conditions}

虽然`if...then...else`语句的条件是一个布尔表达式，但是通常没有必要求出这个表达式的值放入寄存器。大部分机器都提供了条件分支指令(如上面提到的IL指令`brtrue.s`，因为这个表达式求值的目的并不是为了值，而是为了跳转到合适的位置。这种看法使得可以对[#2.5 短路求值](#2.5.short-circuit-evaluation)的表达式生成高效的代码(称为**跳转码**)。跳转码不但可以用于选择语句，也可用在**逻辑控制的循环**中。如下面代码 : 
...
## 5.2 case/switch语句 {#5.2.case-switch-statements}
...
## 6.1 枚举控制的循环 {#6.1.enumeration-controlled-loops}
...
## 6.3 逻辑控制的循环 {#6.3.logically-controlled-loops}
...
## 7.1 迭代和递归 {#7.1.iteration-and-recursion}
...
## 7.2 应用序和正则序求值 {#7.2.applicative-and-normal-order-evaluation}
