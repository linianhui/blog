...
那么我总结推导出来3个核心的原则：**可理解性**、**可测试性**和**可隔离性**。

## 5.1 可理解性 {#objective-intelligibility}

这条原则看起来很有主观性的倾向，但是其实并不是。

比如说你刚写了一段代码，你觉得容易理解，他看起不容易理解；或者说代码是他按照他的思维写的，他看起来很容易理解，但是到你这里无法一下子理解他的思维，然后你就觉得不好理解。**如果出现了这样的情况，那么则统统都是不可理解的**。这时候你要说了：你要一棍子打死双方啊。是的，正是如此。再回想一下我们的目标是什么？**可维护性！**，这里的维护不单单是说你的代码你来维护，而是大家互相交叉着；你新增了一个功能，后续负责其他的事情去了，那么这时候就由你的队友来负责维护了；或者你接受维护别人的代码。

所以我们需要一个客观上的**可理解性**。那么到底什么才能叫客观？没法度量啊！其实也不复杂，就是看当你读到一段代码的时候，你是否需要额外的思考，额外的脑中维持一个上下文的环境才能明白这段代码的意图，如果需要，那么就是不可理解的。**更简单点说就是这段代码应该让你不用思考就看的明白它的意图**。比如下面的一个小例子，功能是完全等价的，但是差异非常微妙。
...
那么如何确保你真实的行为和你所认为的行为是一致的？那就是**测试**。把你认为的行为也写成代码，去验证你的业务代码执行的时候是不是会根据你给定的输入得到你期望的输出结果。借助自动化的CI，你可以把你在你每次改动代码时把现有的所有测试都运行一遍，然后你就可以获得至少3点收益：

1. 代码真的时按照你认为的行为去执行的。
2. 确保你的改动不会破坏现有的代码行为。
3. 倒逼你的代码进行合理的分解和抽象，不然你很难进行有效的测试。

当然你可能把测试写错了，，，这种概率就小多了吧。况且假设你真的写错了测试，时间久了，这个错误也就变成了**feature**。为什么呢？也许你代码的消费方已经按照它希望的行为去处理了，这时候你贸然把这个bug修复了，结果可能时消费方反而不能正常工作了。这时候这个错误的测试其实也就变成了消费方的一种契约测试。确保你不会把它改对，，，比如C#的类库中有个`DateTime`，在处理时区问题时很多诡异的行为，这时候微软已经无法修正它了，只好再单独新增了一个`DateTimeOffset`，两者共存，慢慢的迁移过去。

## 5.3 可隔离性 {#objective-isolability}

那么现在你可以快速的理解现存的代码了，也可以确保你的新代码不会破坏已有的功能，也确认你的代码行为是你所认为的行为了。是不是就可以愉快的合并代码并且上线发布了？是的，差不多可以了。但是，凡是总有例外，我们无法把全部希望都寄托在我们能严格落实上述两点。总是要有个备用方案对吧？

可隔离性就是这样的一个备选方案，其意图就是隔离你的代码行为，哪怕它就是腐烂变质变成了烂代码不可维护的代码，只要不影响其他的模块，那么就还算是可控的。就像万吨巨轮，底层的隔水舱总是一个个的独立的，一个进水了也不影响其他的，从而避免整体的失控。不至于一颗老鼠屎坏了整个一锅粥。
...
模型匹配的含义是说让你的代码真实的表达实际的业务意图，而且这个意图必须要落实到数据层面，而非代码层面。感觉有点绕噢，什么鬼意思？
