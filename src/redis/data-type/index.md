...
string是一个**二进制安全的**字符串，类似于java的String，但是它是可以修改的。value最大c长度不能超过**512MB**。
...
底层实现：Bitmap底层是[string](#string)类型，因为string最大长度为512MB，故而Bitmap最多可以表示<code>512MB =2<sup>29</sup>byte = 2<sup>32</sup>bit</code>。
