    ...
    if ($JsonFilePath) {
        ...
        if (!$VM) {
            $VM = New-VM `
                ...
                -Path $Json.path `
                -SwitchName $Json.network.switchName
        ...
        Log-NameValue -Name 'hdd.fileSize' -NamePadding $NamePadding -Value ((Get-Item $DVD.Path).Length | Byte-Format)
